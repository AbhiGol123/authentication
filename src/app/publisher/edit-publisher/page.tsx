...
export default function EditPublisherPage() {
  ...
  // Get publisher ID from URL params on client side
  useEffect(() => {
  ...
  const fetchPublisherData = async (id: string) => {
    try {
      ...
      setSubmitError('Failed to load publisher data. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const uploadLogoToSupabase = async () => {
    // If no new file selected, keep existing logo
    if (!logoFile) {
    ...
    try {
      ...
      if (existingLogoUrl) {
        const url = new URL(existingLogoUrl);
        const path = url.pathname.split('/').pop();
        if (path) {
          await supabase.storage.from('publisher-logos').remove([path]);
        }
      }
      ...
      return existingLogoUrl; // Keep existing logo if upload fails
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!publisherId) {
      setSubmitError('Publisher ID is missing.');
      return;
    }
    ...
    try {
      ...
      // Update publisher data in the database
      const { data, error } = await supabase
        .from('publishers')
        .update({
          name: formData.name,
      ...
      // Redirect to publishers list or dashboard after successful update
      router.push('/publisher/list');
    } catch (error) {
      console.error('Error updating publisher:', error);
      setSubmitError('Failed to update publisher. Please try again.');
    } finally {
      ...
      <main className="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <div className="overflow-hidden sm:rounded-lg">
            <div className="mb-8">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Edit publisher</h1>
              <p className="text-gray-600 mt-2">Update the details below to edit the publisher profile</p>
            </div>
